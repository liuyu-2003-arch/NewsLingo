...
import { Upload, Music, FileText, PlayCircle, ArrowLeft, Cloud, AlertCircle, Terminal, Copy, ExternalLink, Check, Image as ImageIcon, Languages, Edit2 } from 'lucide-react';
...
const SetupForm: React.FC<SetupFormProps> = ({ initialData, onCancel, onSuccess }) => {
  ...
  const [loadingStep, setLoadingStep] = useState(''); // detailed loading status
  ...
  // Auto-fill title from filename in create mode
  const handleMediaSelect = (file: File | null) => {
    setMediaFile(file);
    if (file && !title && !isEditMode) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Handle Subtitles (Create: Required / Edit: Optional)
      if (subFile) {
        ...
        // Auto-Translate Step
        if (autoTranslate) {
            setLoadingStep(`Translating ${subtitles.length} lines with AI... (this may take a moment)`);
            try {
                subtitles = await translateSubtitles(subtitles);
      ...
      if (isEditMode && initialData) {
         ...
         await updateSession(initialData.id, title, subtitles, coverFile || undefined);
         onSuccess(initialData.id);
      } else {
         if (!mediaFile || !subtitles) return; // Should be caught by validation above
         
         setLoadingStep('Uploading to Cloud...');
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
                ...
                <input
                    ...
                    className="block w-full px-4 py-3 rounded-lg border-gray-300 bg-slate-50 border focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-slate-700">
                {isEditMode ? 'Change Cover Image (Optional)' : 'Cover Image (Optional)'}
              </label>
              
              {/* Show existing cover if in edit mode and no new file selected */}
              {isEditMode && !coverFile && initialData?.coverUrl && (
...
create policy "Public Access" on sessions for all to anon using (true) with check (true);`}
            ...
            >
              {loading ? (loadingStep || 'Processing...') : (isEditMode ? 'Save Changes' : 'Upload & Sync')}
            </button>
          </form>
        </div>
      </div>
